...
class {0} : public quiet::script::entity_script
{{
public:
	constexpr explicit {0}(quiet::game_entity::entity entity)
		: quiet::script::entity_script{{entity}} {{}}
...
private:
        ...
        bool Validate()
        {
            ...
            else if (!Path.GetFullPath(Path.Combine(Project.Current.Path, path)).Contains(Path.Combine(Project.Current.Path, @"Code\")))
            {
                errorMsg = "Script must be added to (a sub-folder of) Code";
            }
