...
namespace QEditor.Components
{
    ...
    sealed class MSTransform : MSComponent<Transform>
    {
        ...
        protected override bool UpdateMSComponents()
        {
            ...
            PosZ = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(z => z.Position.Z));

            RotX = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(x => x.Rotation.X));
            RotY = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(y => y.Rotation.Y));
            RotZ = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(z => z.Rotation.Z));

            ScaleX = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(x => x.Scale.X));
            ScaleY = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(y => y.Scale.Y));
            ScaleZ = MSEntity.GetMixedValue(SelectedComponents, new Func<Transform, float>(z => z.Scale.Z));
