...
namespace quiet::graphics::d3d12::core
{
	namespace
	{
		...
		public:
			...
			explicit d3d12_command(ID3D12Device10* const device, D3D12_COMMAND_LIST_TYPE type)
			{
				...
				NAME_D3D12_OBJECT(_cmd_queue,
					type == D3D12_COMMAND_LIST_TYPE_DIRECT ? L"Graphics Command Queue" :
					type == D3D12_COMMAND_LIST_TYPE_COMPUTE ? L"Compute Command Queue" :
					L"Command Queue"
				);
				...
				for (u32 i{ 0 }; i < frame_buffer_count; ++i)
				{
					...
					NAME_D3D12_OBJECT_INDEXED(frame.cmd_allocator, i,
						...
						type == D3D12_COMMAND_LIST_TYPE_COMPUTE ? L"Compute Command Allocator" :
						L"Command Allocator");
				}
				...
				NAME_D3D12_OBJECT(_cmd_list,
					type == D3D12_COMMAND_LIST_TYPE_DIRECT ? L"Graphics Command List" :
		...
		void __declspec(noinline) process_deferred_releases(u32 frame_idx)
		{
			...
			// We clear this flag in the beginning. If we'd clear it at the end then it might overwrite some other thread that was trying to set it.
	...
	void shutdown()
	{
		...
		// We don't call process_deferred_releases at the end because some resources (such as swap chains) can't be released before
		// their dependent resources are released.
		for (u32 i{ 0 }; i < frame_buffer_count; ++i)
		{
		...
		// Some types only use deferred release for their resources during shutdown/reset/clear.
