...
namespace quiet::content
{
    namespace
    {
        ...
        bool read_script(const u8*& data, game_entity::entity_info& info)
        {
            ...
            // if a script name is longer than 255 characters then something is probably wrong
            // either with the binary writer or with the game programmer...
	...
	bool load_game()
	{
        ...
        for (u32 entity_index{ 0 }; entity_index < num_entities; ++entity_index)
        {
            ...
            assert(info.transform); // if info contains transform
